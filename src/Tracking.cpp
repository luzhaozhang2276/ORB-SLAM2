...
void Tracking::Track()
{
    // track包含两部分：估计运动、跟踪局部地图
            // 运动估计:TrackReferenceKeyFrame(),TrackWithMotionModel(),Relocalization()
            // 局部跟踪:TrackLocalMap()
    ...
    else/// 步骤2：跟踪
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                if(mVelocity.empty() || mCurrentFrame.mnId<mnLastRelocFrameId+2)
                {
                    ...
                    // (课件中:将上一帧与当前帧进行BoW匹配,这样的表述是不对的)
                    // (此处应为:当前帧与参考关键帧之间的匹配,且将当前帧位姿设为上一帧位姿,以获得较好的优化初值)
                    bOK = TrackReferenceKeyFrame();
                    // TO DO: VO中当前帧只与参考关键帧(关键帧)进行跟踪,不对上一帧进行跟踪???  TrackWithMotionModel 对上一帧进行跟踪
                }
                else
                {
                    // 根据恒速模型设定当前帧的初始位姿
            ...
            {
                // BOW搜索，PnP求解位姿
                bOK = Relocalization();     // 进行重定位,并返回标志位
            }
        }
        else
        {
            ...
            /// 步骤2.1：跟踪上一帧或者参考帧或者重定位
            // 这个else中是Localization Mode中的,同样也包含三种情况(上一帧,参考帧,重定位)
            ...
            else
            {
                ...
                if(!mbVO)       // 正常跟踪
                {
                    ...
                    {
                        bOK = TrackReferenceKeyFrame(); // 参考帧
                    }
                }
                else            // 跟踪数目太少       既跟踪又定位
                {
        ...
        if(!mbOnlyTracking)     // 正常跟踪模式(具有局部地图)
        {
            if(bOK)
                bOK = TrackLocalMap();  // 不论成功与否,都对pose进行了优化
        }
        else    // Localization Mode : 只有重定位成功后,才进行局部地图跟踪
        {       // TODO :该模式中,"局部地图不工作"指的是不进行更新??? 下边的LocalMap是原地图中的信息???
        ...
        if(bOK)
        {
            ...
            mpMapDrawer->SetCurrentCameraPose(mCurrentFrame.mTcw);      // 相机位姿可视化

            // Clean VO matches
            // 临时添加的MapPoints只用于跟踪,结束后需清除
            /// 步骤2.4：清除UpdateLastFrame中为当前帧临时添加的MapPoints
            for(int i=0; i<mCurrentFrame.N; i++)        // 删除在当前帧内增加的MapPoints,数据库中在下一步骤进行删除
            {
                ...
                if(pMP)
                    ...
                    if(pMP->Observations()<1)
                    {
                        ...
                        mCurrentFrame.mvpMapPoints[i]=static_cast<MapPoint*>(NULL);     // 清除
                    }
            }
...
void Tracking::StereoInitialization()
{
    ...
    if(mCurrentFrame.N>500)
    {
        ...
        // 添加MapPoints,并映射到对应的关键帧
        for(int i=0; i<mCurrentFrame.N;i++)
        {
            float z = mCurrentFrame.mvDepth[i];     // 获取深度值
            if(z>0)
            {
                // 步骤4.1：通过反投影得到该特征点的3D坐标
                cv::Mat x3D = mCurrentFrame.UnprojectStereo(i);
                // 步骤4.2：将3D点构造为MapPoint
                // 使用MapPoint的构造函数 (世界坐标,KeyFrame,Map)
...
void Tracking::MonocularInitialization()
{
    ...
    if(!mpInitializer)
    {
        ...
        if(mCurrentFrame.mvKeys.size()>100)
        {
            ...
            // 记录最近的一帧
            mLastFrame = Frame(mCurrentFrame);  // TO DO: 这么写,两帧不是一样的吗??? 是一样的,但是不是用这两帧来初始化
                                                // 是用mInitialFrame与mCurrentFrame进行初始化,mCurrentFrame会更新
                                                // mLastFrame 的用途呢??? 在单目初始化中没用上,在Tracking中用上了
            ...
            mpInitializer =  new Initializer(mCurrentFrame,1.0,200);    // 创建单目初始器
    ...
    else
    {
        ...
        // 主要函数: Initialize() 并行计算H和F模型,只返回评分高的模型
        if(mpInitializer->Initialize(mCurrentFrame, mvIniMatches, Rcw, tcw, mvIniP3D, vbTriangulated))
        {
            /// 步骤6：删除那些无法进行三角化的匹配点
            for(size_t i=0, iend=mvIniMatches.size(); i<iend;i++)   // 遍历初始匹配对
            {
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    /// 步骤3：将关键帧插入到地图
    // 凡是关键帧，都要插入地图     TO DO: 局部地图 or 全局地图 ??? 全局地图
    mpMap->AddKeyFrame(pKFini);
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    /// 步骤4:通过优化3D-2D的重投影误差来获得位姿 (pnp?)
    // pose-only BA, 不优化MapPoints,但是能剔除MapPoints的outliers
...
bool Tracking::TrackWithMotionModel()
{
    ...
    // mVelocity为最近一次前后帧位姿之差,在每次track的最后进行更新
    ...
    // 此处的th决定了SearchByProjection的搜索半径,threshold; 双阈值匹配(搜索)
    int th;
    ...
    // SearchByProjection是3D点与2D点之间的匹配
    // ORBmatcher.cpp共包括4个重载的SearchByProjection函数,用于不同的情形
    ...
    // TODO: 参考关键帧模型和恒速运动模型的g2o求解器有什么区别?参数一类的,哪些参数?
...
bool Tracking::TrackLocalMap()
{
    ...
    // 若为正常跟踪:
            // 1.相机位姿跟踪时,进行位姿优化:TrackReferenceKeyFrame()、TrackWithMotionMode()
            // 2.局部地图点与当前帧匹配,又进行位姿优化:TrackLocalMap()
    ...
    for(int i=0; i<mCurrentFrame.N; i++)    // 遍历当前帧的MapPoints
    {
        if(mCurrentFrame.mvpMapPoints[i])
        {
            // 由于当前帧的MapPoints可以被当前帧观测到，其被观测统计量加1
            if(!mCurrentFrame.mvbOutlier[i])    // bool量,用于判断是否能观测到
            {
                mCurrentFrame.mvpMapPoints[i]->IncreaseFound();
                if(!mbOnlyTracking)
                {
    ...
    if(mnMatchesInliers<30)
        ...
        return true;
    // 不论成功与否,都对pose进行了优化,因为步骤三中对优化函数进行了调用PoseOptimization()
}
...
void Tracking::SearchLocalPoints()
{
    ...
    for(vector<MapPoint*>::iterator vit=mvpLocalMapPoints.begin(), vend=mvpLocalMapPoints.end(); vit!=vend; vit++)
    {
        ...
        if(pMP->mnLastFrameSeen == mCurrentFrame.mnId)      // 即步骤1中的MapPoints
            continue;
        if(pMP->isBad())
            continue;
    ...
    if(nToMatch>0)
    {
        ...
        matcher.SearchByProjection(mCurrentFrame,mvpLocalMapPoints,th);     // 匹配结果存于Frame中
    }
}
...
void Tracking::UpdateLocalMap()
{
    ...
    mpMap->SetReferenceMapPoints(mvpLocalMapPoints);        // 用于可视化操作
...
void Tracking::UpdateLocalPoints()
{
    ...
    {
        KeyFrame* pKF = *itKF;
        const vector<MapPoint*> vpMPs = pKF->GetMapPointMatches();  // 获取该关键帧的Mappoints
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    for(int i=0; i<mCurrentFrame.N; i++)    // 遍历次数为当前帧的MapPoints数量
    {
        if(mCurrentFrame.mvpMapPoints[i])   // 遍历当前帧的MapPoints
        {
            MapPoint* pMP = mCurrentFrame.mvpMapPoints[i];  // 取出3D点
            if(!pMP->isBad())   // 不是坏点
            {
                // 遍历能观测到当前帧MapPoints的关键帧
                const map<KeyFrame*,size_t> observations = pMP->GetObservations();
                for(map<KeyFrame*,size_t>::const_iterator it=observations.begin(),
                        itend=observations.end();
                        it!=itend; it++)
                    keyframeCounter[it->first]++;   // first对应的是KeyFrame指针
            }
            else
            {
    ...
    // 遍历所有能观测到当前帧MapPoints的关键帧,添加到局部关键帧mvpLocalKeyFrames中
    ...
    // 共视程度高的前10帧 + 子关键帧 + 父关键帧
    // 遍历策略一中的局部关键帧
    for(vector<KeyFrame*>::const_iterator itKF=mvpLocalKeyFrames.begin(), itEndKF=mvpLocalKeyFrames.end(); itKF!=itEndKF; itKF++)
    {
        // Limit the number of keyframes        策略一中据局部关键帧的数量不能过高
        ...
        /// 策略2.1:最佳共视的10帧
        const vector<KeyFrame*> vNeighs = pKF->GetBestCovisibilityKeyFrames(10);    // 仅获取,此处并不进行计算(降序集合)
        ...
        /// 策略2.2:自己的子关键帧
        const set<KeyFrame*> spChilds = pKF->GetChilds();       // 此处为关键帧中,生成树的相关函数
        for(set<KeyFrame*>::const_iterator sit=spChilds.begin(), send=spChilds.end(); sit!=send; sit++) // 遍历子关键帧
        {
            KeyFrame* pChildKF = *sit;
            if(!pChildKF->isBad())      // 不是坏点
            {
                // 防止重复添加局部关键帧
                if(pChildKF->mnTrackReferenceForFrame!=mCurrentFrame.mnId)  // 孩子关键帧跟踪的参考关键帧不是当前帧
                {
                    mvpLocalKeyFrames.push_back(pChildKF);  // 添加该孩子节点为局部关键帧
        ...
        /// 策略2.3:自己的父关键帧
        KeyFrame* pParent = pKF->GetParent();       // 父关键帧只有一个,不需要循环遍历
        if(pParent)
        {
            // mnTrackReferenceForFrame防止重复添加局部关键帧
